"""
Frontend UI for the ROS Component Explorer.
Provides a web-based interface with a natural language search interface (UI only - no actual LLM processing).
The application displays ROS components from TTL/RDF files through Apache Solr indexing.

Note: The natural language processing uses rule-based pattern matching
and structured templates, not external AI/machine learning models.
"""
...
class ROSComponentUI:
    """
    Main UI class for the ROS Component Explorer.
    
    Provides a component browser with natural language search capabilities using
    rule-based pattern matching and semantic search. The system processes user queries
    through structured templates and keyword extraction to find relevant ROS components.
    """
    ...
    def __init__(self, db_manager, ttl_file: str = None):
        ...
        # Initialize natural language search engine (rule-based pattern matching system)
        try:
            # Import here to avoid dependency issues if NLP modules aren't available
            import sys
            import os
            ...
            logger.info("Rule-based natural language search engine initialized")
        except Exception as e:
            logger.warning(f"Natural language search engine not available: {e}")
    ...
    def build_ui(self):
        """Build the complete user interface with rule-based natural language processing."""
        try:
            # Main container
    ...
    def _perform_nlp_search(self, e=None):
        """
        Perform natural language search using rule-based pattern matching.
        
        NOTE: Despite the name "natural language search", this uses structured 
        pattern matching and keyword extraction, not external AI/ML services.
        """
        ...
        try:
            ...
            if hasattr(self, 'nlp_response_container'):
                ...
                with self.nlp_response_container:
                    ...
                    ui.label('Processing your query using rule-based pattern matching...').classes('text-gray-600')
                self.nlp_response_container.classes(remove='hidden')
            
            # Process query with rule-based pattern matching asynchronously
            async def process_query():
                try:
                    # Uses structured pattern matching and keyword extraction
                    result = self.nlp_engine.process_natural_language_query(query, max_results=10)
                    
                    # Update NLP response
                    if hasattr(self, 'nlp_response_container'):
                        self.nlp_response_container.clear()
                        with self.nlp_response_container:
                            # Show structured response (generated by rule-based templates)
                            with ui.card().classes('w-full p-4 bg-blue-50 border-l-4 border-blue-500'):
                                ui.label('Search Response').classes('font-semibold text-blue-800 mb-2')
                    ...
                    logger.error(f"Pattern matching search error: {e}")
                    if hasattr(self, 'nlp_response_container'):
                        self.nlp_response_container.clear()
                        with self.nlp_response_container:
...
def build_ui(db_manager, ttl_file: str = None):
    """Build and return the complete UI with rule-based natural language processing."""
