#!/usr/bin/env python3
"""
Command-line demo of the Rule-Based Natural Language ROS Component Explorer.
...
def interactive_demo():
    """Run an interactive demo of the rule-based natural language search engine."""
    print("🚀 ROS Component Explorer - Natural Language Search Demo")
    print("=====================================")
    print()
    
    # Initialize the search engine with mobile robot components
    mobile_robot_file = "data/mobile_robot_components.ttl"
    legacy_files = [
        "data/components_converted.ttl",
        "data/components.ttl",
        "/home/ritz/ros-component-explorer/data/components_converted.ttl",
        "/home/ritz/Desktop/RnD/data/components.ttl"
    ]
    
    ttl_file = None
    if os.path.exists(mobile_robot_file):
        ttl_file = mobile_robot_file
        print(f"🤖 Using mobile robot components: {ttl_file}")
    else:
        for legacy_file in legacy_files:
            if os.path.exists(legacy_file):
                ttl_file = legacy_file
                print(f"⚠️ Using legacy components: {ttl_file}")
                break
    
    if not ttl_file:
        print("❌ No data files found! Please ensure data files exist.")
        print("Expected files:")
        print(f"  - {mobile_robot_file} (preferred)")
        print("  - data/components_converted.ttl (fallback)")
        return
    ...
    try:
        ...
        print(f"❌ Failed to initialize Natural Language Search Engine: {e}")
        return
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("\n👋 Thanks for trying the ROS Component Explorer Natural Language Search demo!")
            break
...
def batch_demo():
    ...
    print("🚀 ROS Component Explorer - Batch Natural Language Search Demo")
    print("==========================================")
    print()
